"""
Training script for SplitMNIST with Modular Continual Learning.

This script trains a modular network with attention routing on SplitMNIST
using Synaptic Intelligence (SI) for continual learning. The goal is to
reach >97% average accuracy in task-IL setting.

The network uses:
- ModularLayer with attention routing
- Synaptic Intelligence (SI) for parameter regularization
- Low router regularization coefficients for stability
- Task-agnostic routing without task labels
"""
...
class ModularContinualLearner(ContinualLearner):
    ...
    def __init__(self, input_size=784, hidden_sizes=[400, 400], num_classes=10, 
    ...
    def compute_importance_weights(self, dataloader, device):
        """Compute importance weights for SI using a simplified approach."""
        self.eval()
        importance_weights = {}
        ...
        # Use a subset of data for importance computation
        for batch_idx, (data, target) in enumerate(dataloader):
            ...
            for (name, param), grad in zip(self.named_parameters(), gradients):
                if param.requires_grad:
                    importance_weights[name] += grad ** 2
    ...
    def si_regularization_loss(self):
        ...
        for name, param in self.named_parameters():
            if param.requires_grad and name in self.importance_weights:
                if name in self.old_params:
                    ...
                    reg_loss += (self.importance_weights[name] * diff ** 2).sum()
        
        return self.epsilon * reg_loss
    
    def update_old_params(self):
        """Update old parameters for SI."""
        self.old_params = {}
        for name, param in self.named_parameters():
            if param.requires_grad:
...
def train_task(model, train_data, train_labels, device, epochs=10, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(dataloader):
            ...
            # SI regularization
            si_loss = model.si_regularization_loss()
            
            # Total loss
            total_loss_batch = ce_loss + router_loss + si_loss
...
def main():
    ...
    print("Training Modular Continual Learning on SplitMNIST")
    print("=" * 60)
    ...
    for task_id in range(len(train_tasks)):
        ...
        print("Training...")
        train_task(model, train_data, train_labels, device, epochs=20, lr=0.001)
        
        # Update old parameters for SI (before computing importance weights)
        model.update_old_params()
        
        # Compute importance weights for SI (after training is complete)
        print("Computing importance weights...")
        dataset = TensorDataset(train_data.float() / 255.0, train_labels)
        dataloader = DataLoader(dataset, batch_size=128, shuffle=False)
        model.compute_importance_weights(dataloader, device)
    ...
    if final_avg_acc > 0.97:
        ...
        print(f"❌ Did not reach 97% target (got {final_avg_acc:.4f})")
